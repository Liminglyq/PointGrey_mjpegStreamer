...
typedef struct {
    ...
    BusManager busMgr;  //get numbers of cameras ;unsigned int numCameras;	error = busMgr.GetNumOfCameras(&numCameras);
	PGRGuid guid;		//get camera from index ;error = busMgr.GetCameraFromIndex(0, &guid);
	Camera cam;         // Connect to a camera;  error = cam.Connect(&guid);
...
int input_init(input_parameter *param, int plugin_no)
{
    ...
    try {
    	//device use as default
        if (!strcasecmp(device, "default")) {
            pctx->capture.open(0);
            pctx->busMgr.GetNumOfCameras(&numCameras);
            if ( numCameras < 1 ){cout << "Insufficient number of cameras... exiting" << endl;return -1;}
            pctx->busMgr.GetCameraFromIndex(0, &(pctx->guid));//check if & need 
			pctx->cam.Connect(&(pctx->guid));
			pctx->cam.StartCapture();
...
void *worker_thread(void *arg)
{
    ...
    while (!pglobal->stop) {
        ...
        if (!pctx->capture.read(test))
            break; // TODO
         
        Image rawImage;
		// Retrieve an image
		pctx->cam.RetrieveBuffer( &rawImage );
		...
		cv::Mat src = cv::Mat(rgbImage.GetRows(),rgbImage.GetCols(),CV_8UC3,rgbImage.GetData(),rowBytes);
